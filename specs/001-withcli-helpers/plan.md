...
Strictly additive: every currently exported symbol survives unchanged.
...
lib/
  GitHub/
    Release/
      Check.hs                   # ← re-exports CliBanner + withCli; rest unchanged
      Check/
        Cli.hs                   # NEW — CliBanner type + withCli
...
**Structure Decision**: stay with the existing single-library layout. The new module `GitHub.Release.Check.Cli` is a sibling of `Cache` / `Decision` / `Fetcher` and is re-exported from the umbrella `GitHub.Release.Check` for consumer ergonomics (one import). The new sublibrary `github-release-check:optparse` adds one Cabal section and one new module `GitHub.Release.Check.OptParse`. No new top-level directories.
...
| `lib/GitHub/Release/Check/Cli.hs`, `test/GitHub/Release/Check/CliSpec.hs`, cabal hunks for the core lib | **Subagent** (slice S1) |
...
### S1 — `withCli` + `CliBanner` in `GitHub.Release.Check.Cli`

**Subject**: `feat(cli): add CliBanner + withCli helper`
**Tasks** (to be enumerated by `/speckit.tasks`): one RED-GREEN pair plus the re-export.
**Files**: `lib/GitHub/Release/Check/Cli.hs` (new), `lib/GitHub/Release/Check.hs` (export list + re-export only), `test/GitHub/Release/Check/CliSpec.hs` (new), `github-release-check.cabal` (add `Cli` to `exposed-modules`, add `CliSpec` to test `other-modules`).
...
|---|---|---|
| S1 | `CliSpec` is added importing `GitHub.Release.Check (CliBanner (..), withCli)`. **Compile fails**: module/type/function absent. Test cases assert: (a) env unset → `cfgDisabled = False` after composition; (b) env set → `cfgDisabled = True` regardless of modifier; (c) modifier mutates a non-`cfgDisabled` field and the change is visible in the composed `Config`; (d) order: `defaultConfig` → modifier → env-var override. To enable inspection, S1 also exports `composeCliConfig :: CliBanner -> (Config -> Config) -> IO Config` from `GitHub.Release.Check.Cli` (and re-exported); `withCli` is then defined as `composeCliConfig b f >>= \cfg -> withUpdateCheck cfg action`. | `Cli.hs` defines `CliBanner`, `composeCliConfig`, `withCli`; re-export added; tests pass. |
